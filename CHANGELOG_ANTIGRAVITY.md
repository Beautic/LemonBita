...
- **구현 내용**:
  - 마이벤토리의 브랜드 리뉴얼 테마에 맞추어, 후보 시안 중 정리함/수납의 직관적인 은유를 담은 **컨셉 C (오픈 박스)**를 최종 공식 앱 아이콘으로 지정했습니다.
  - [generate.py](file:///Users/a421104/Documents/project/Antigravity/dress/app_icons/myventory/generate.py) 아이콘 빌더 스크립트를 고도화하여, 활성 컨셉(c-open-box)에 대응되는 5종의 필수 모바일/네이티브 아이콘 및 5종의 웹 전용 아이콘을 `qlmanage`를 통해 자동으로 1024px/512px/192px/32px 고해상도 PNG 파일로 렌더링하고 목적지 경로에 정밀 배치하는 인프라를 구축했습니다.
  - `flutter_launcher_icons` 도구를 실행하여 Android/iOS 네이티브 프로젝트 폴더의 런처 아이콘 에셋을 일괄 재생성 및 동기화했습니다.
  - 웹 폴더 하위의 `web/favicon.png` 및 `web/icons/Icon-192.png`, `web/icons/Icon-512.png`, `web/icons/Icon-maskable-192.png`, `web/icons/Icon-maskable-512.png` 에셋을 컨셉 C에 해당하는 이미지로 강제 덮어쓰기 적용하여 브라우저 파비콘 및 PWA 설치 아이콘을 일관되게 단일화했습니다.

### 2. 개발계(dev) 빌드 및 Firebase 배포 적용
- **구현 내용**:
  - 신규 아이콘이 완벽히 녹아든 상태에서 `./deploy_dev.sh` 스크립트를 실행하여 Flutter Web 릴리즈 빌드 및 Firebase Hosting 개발계 배포 프로세스를 성황리에 완료했습니다.

---

## 2026-07-13 (v6.1.2) — 의류 그리드 내 '🧼 세탁 필요' 파란색 알림 배지 원본 가시성 복원


### 1. 홈 화면 인벤토리 슬롯 내 '🧼 세탁 필요' 배지 복원 및 테마 융합
- **구현 내용**:
  - 홈 화면의 격자 아이템 리뉴얼 당시 공간 다이어트 과정에서, 세탁이 필요한 의류에 표시되던 기존 파란색 배지가 '좌하단 붉은색 미세 도트(3px)'로 과도하게 축소되어 가시성이 크게 저하되었던 피드백을 수용했습니다.
  - 이전 버전의 높은 직관성과 사용성을 완벽히 계승하도록, 의류 이미지(옷) 영역 좌측 하단에 겹쳐 뜨는 선명한 파란색 `🧼 세탁 필요` 꼬마 배지([home_screen.dart](file:///Users/a421104/Documents/project/Antigravity/dress/lib/screens/home_screen.dart))를 정합 복원했습니다.
  - 마이벤토리의 격자 테마 컨셉에 조화되도록 둥글둥글한 라벨 대신 엣지 있는 사각형 디자인(`BorderRadius.circular(4)`) 및 8.5px 볼드 텍스트와 그림자 효과를 가미하여 가독성을 높였습니다.
...
- **구현 내용**:
  - [friend_closet_screen.dart](file:///Users/a421104/Documents/project/Antigravity/dress/lib/screens/friend_closet_screen.dart) 파일을 `StatefulWidget`으로 리팩토링하고 탭 전환 상태(`_selectedTab`)를 추가했습니다.
  - 상단에 수평형 슬라이더 느낌의 탭 바를 추가하여 `아이템 목록`과 `추천한 코디`를 즉각 전환할 수 있도록 구현했습니다.
  - **아이템 목록 탭**: 홈화면 인벤토리 그리드와 통일성 있게 `AppColors.slot` 음각 배경이 입혀진 **3열 1:1 정사각 격자 슬롯**으로 친구 옷을 표출하여 디자인 톤을 일치시켰습니다.
  - **추천한 코디 탭**: 내가 해당 친구에게 추천 및 선물한 코디 내역(`suggestedById == currentUserId`)을 실시간 동기화하여 **2열 4:5 매거진 비율 그리드**로 시각적으로 시원하게 노출하고, 탭 시 상세 보기(`PlannedOotdDetailScreen`)로 연계시켰습니다.

### 2. 친구 검색 RTT 레이턴시 극감 및 시각 피드백 추가
- **구현 내용**:
  - `searchUsers` 내에서 `email`, `phoneNumber`, `nickname`에 대한 3중 순차 Firestore get 요청을 비동기 병렬 구조인 `Future.wait`로 묶어 처리함으로써, 통신 지연 시간을 기존 대비 **1/3 수준**으로 대폭 단축했습니다.
  - 검색 중(대기 상태)일 때 입력 박스 바로 하단에 얇은 `LinearProgressIndicator(color: Colors.black)`를 노출하여 시각적 지루함을 줄이고 체감 성능을 향상했습니다.

### 3. 닉네임 지능형 부분 검색 (Prefix Match Query) 적용
- **구현 내용**:
  - 기존 닉네임 검색 시 완벽히 일치하는 단어만 필터링되던 `isEqualTo` 검색의 성능 한계를 극복했습니다.
  - 사용자가 앞 글자만 쳐도 관련 이름이 전부 색인되도록 `isGreaterThanOrEqualTo` 및 `isLessThanOrEqualTo` 범위 기반 Prefix 조회를 적용하여 검색 사용성을 크게 확대했습니다.

---

## 2026-07-13 (v6.0.0) — 마이벤토리(Myventory) UI 리뉴얼 및 브랜드 리브랜딩 대규모 업데이트

### 1. 테마 디자인 토큰 및 가독성 개선 시스템 도입 (`lib/theme/app_theme.dart` [NEW])
- **구현 내용**:
...
### 2. 홈 화면 1:1 정사각 슬롯형 인벤토리 그리드 구성
- **구현 내용**:
  - 기존 3열 옷장 그리드를 **정사각 슬롯형(childAspectRatio: 1.0)** 격자로 리뉴얼하여 이종 소품 수납 준비.
...
### 4. 프로필 대시보드 전개 통합 (`profile_screen.dart` 리팩토링)
- **구현 내용**:
...
### 5. OOTD 그리드 크롭 버그 해결
- **구현 내용**:
  - 레터박스 여백으로 찌그러지던 피드 목록 이미지를 정사각 커버 크롭(`BoxFit.cover`)으로 전면 교정.

### 6. 카피 중립화 및 리브랜딩 전면 적용
- **구현 내용**:
  - 옷장/옷 -> 인벤토리/아이템 단어 교체.
...
- **구현 내용**:
  - 기존 자동 정렬 시 원피스나 드레스 의류들이 기타(Accessory) 카테고리로 오분류되는 알고리즘 문제를 정정하여 `onepiece` 전용 분석 및 매핑 과정을 확보했습니다.
  - 여성 사용자가 원피스 밑에 바지(하의, `bottom`)를 매치해 덧입는 레이어드 룩의 경우, 원피스의 Y축 중심을 위쪽(약 30%), 바지의 Y축 중심을 중간 아래쪽(약 60%)으로 상호 겹쳐 배치하여 원피스 밑단 밖으로 바지가 흘러나오도록 밸런스를 개선했습니다.
  - Z-Index 랭킹 알고리즘(`_getArrangementRank`)을 개편하여 원피스가 항상 하의보다 위에 얹혀 렌더링되게 랭크 순서(`bottom`: 2, `onepiece`: 3)를 재설계했습니다.

### 2. 아이폰(iOS Safari) 웹 환경 아이콘 깨짐 및 미표출 현상 해결
- **구현 내용**:
  - Flutter CanvasKit 렌더러가 iOS 모바일 브라우저(Safari) 환경에서 Google Web Fonts 리소스(Material Icons)를 간헐적으로 차단하거나 렌더링하지 못하는 버그를 확인했습니다.
  - 모바일 환경에서의 폰트 호환성 및 네트워크 속도 극대화를 위해 Flutter Web 빌드 옵션을 `--web-renderer html` 로 전면 전환했습니다.
  - [index.html](file:///Users/a421104/Documents/project/Antigravity/dress/web/index.html) 내부 `<head>` 영역에 구글 원격 Material Icons 폰트 스타일 링크와 `.material-icons`에 대한 font-family 강제 바인딩 CSS 설정을 삽입하여 아이폰 웹 뷰 환경에서도 아이콘이 100% 깔끔하게 출력되도록 교정했습니다.

---

## 2026-07-12 (v5.3.2) — 자동 정렬 완료 스낵바 문구 내 불필요한 기하학적 멘트 수정

### 1. 코디 캔버스 자동 정렬 완료 메시지 순화
- **구현 내용**:
  - 코디 캔버스([CoordinationCanvasScreen](file:///Users/a421104/Documents/project/Antigravity/dress/lib/screens/coordination_canvas_screen.dart))에서 자동 정렬 마법봉 클릭 완료 시 노출되는 스낵바 텍스트를 개선했습니다.
  - 지나치게 수식적이거나 부자연스럽던 `'코디 아이템들이 기하학적으로 자동 정렬되었습니다! ✨'` 문구에서 '기하학적으로' 멘트를 지워내어 훨씬 더 깔끔하고 고급스러운 **`'코디 아이템들이 자동 정렬되었습니다.'`**로 정돈했습니다.
