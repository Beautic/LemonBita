# 디지털 옷장 (My Digital Closet) 작업지시서 v3.8

본 문서는 **v3.6 작업지시서** 위에서 **색깔 태깅 입력 UI 도입(v3.7)** 및 **옷 상세 페이지 내 이미지 수정 기능 추가(v3.8)**를 반영한 변경점만 명시합니다. v3.6 이하의 모든 사양은 그대로 유효하며, 본 문서가 충돌하는 항목만 덮어씁니다.

(v3.7 변경점: ① 옷 등록(`upload_screen`) 및 상세(`clothing_detail_screen`) 화면에 **19색 프리셋 + 직접입력** ChoiceChip UI 도입 ② Firestore `clothing` 도큐먼트에 `color` 필드 정식 사용 ③ `FirebaseService.saveClothingData` 시그니처에 optional `color` 파라미터 추가 ④ 입력 색상 집합(19) ↔ 검색 색상 집합(12) 불일치는 차기 정리 항목으로 명시.)
(v3.8 변경점: ① 옷 상세 페이지(`clothing_detail_screen`)에서 등록된 이미지 변경 기능 추가 ② `image_picker` 카메라/앨범 연동 및 변경된 사진에 대한 누끼 제거/복원 지원.)
...
## 6. 옷 상세 페이지 내 이미지 수정 기능 추가 (v3.8 신규)

### 6.1 배경

사용자가 이미 등록한 옷의 사진이 마음에 들지 않거나, 새로 사진을 찍어 갱신하고 싶을 때 기존에는 해당 옷을 지우고 새로 등록해야 하는 불편함이 있었습니다.
v3.8에서는 `clothing_detail_screen.dart`에 `image_picker`를 연동하여 등록된 사진을 쉽게 교체할 수 있는 기능을 추가합니다.

### 6.2 기능 설명

- 옷 상세 페이지(`clothing_detail_screen.dart`)의 메인 이미지 프리뷰에 "수정(Edit)" 아이콘 버튼 추가.
- 아이콘 클릭 시 `BottomSheet`를 통해 '카메라로 촬영', '앨범에서 선택' 옵션 제공.
- 새 사진 선택 시 화면에 즉시 로컬 이미지(`_localImageBytes`)로 프리뷰 업데이트.
- 기존의 "현재 이미지 누끼 제거하기", "원본 이미지로 복원하기" 기능이 새로 선택한 로컬 이미지에 대해서도 정상 동작하도록 지원.
- 최종 이미지(누끼 제거된 결과물 또는 원본 로컬 이미지)는 화면 하단의 '정보 저장하기' 버튼을 누를 때에만 클라우드(Firebase Storage 및 Firestore)에 반영.
