# 디지털 옷장 (My Digital Closet) 작업지시서 v4.2

본 문서는 **v4.1 작업지시서** 위에서 추가로 설계 및 구현된 **누끼 이미지 정사각형 정규화 및 OOTD 비율 유지** 내역을 세세하게 기록합니다. 
...
### 4.4 누끼 이미지 정사각형 정규화 (v4.2 신규)
- **목적**: 누끼가 제거된 이미지의 비율이 옷마다 제각각이어서 리스트나 그리드 뷰에서 일관성이 떨어지는 문제 해결.
- **구현 로직**: `bg_removal_service.dart`의 배경 제거 후처리 로직(`_cropToSquare`)에 `image` 패키지를 도입. AI 배경 제거 및 오토 크롭(Auto-Cropping)을 마친 이미지를, 옷 원본 픽셀 기준으로 10%의 여백을 둔 1:1 정사각형 캔버스 중앙에 배치하여 최종 저장하도록 개선.

### 4.5 전체 앱 이미지 렌더링 비율 맞춤 (v4.2 신규)
- **목적**: OOTD 기록용 전신 사진이나 긴 치마, 원피스 같은 옷 썸네일 이미지가 `BoxFit.cover` 속성 때문에 크롭(Crop)되어 잘려 보이는 현상을 앱 전역에서 해소.
- **구현 로직**: OOTD 피드 화면, 내 OOTD 상세 화면, 친구 피드, 코디 아이디어 화면, OOTD 달력, 옷장 검색 화면 등 앱 전반에 걸쳐 사용된 `BoxFit.cover`를 모두 `BoxFit.contain`으로 일괄 변경.
