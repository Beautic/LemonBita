...
# ─── 1. 인증 (firebase_auth SDK) ──────────────────────────────
echo ""
echo "[1/8] 인증 (firebase_auth SDK — v3.5부터 SDK가 세션 자체 영속화)"

FBS="lib/services/firebase_service.dart"
check "firebase_service.dart 존재" "$(file_exists $FBS)"
check "firebase_auth import" "$(has_pattern $FBS 'firebase_auth')" "SDK가 빠지면 인증 전체 동작 불가"
check "authStateChanges 리스너 등록" "$(has_pattern $FBS 'authStateChanges\(\)\.listen')" "이게 없으면 앱 시작 시 로그인 상태 복원 안 됨"
check "signUpWithEmail에서 SDK 호출" "$(has_in_function $FBS 'signUpWithEmail' 'createUserWithEmailAndPassword')" "회원가입이 SDK를 거치지 않으면 세션 생성 안 됨"
check "loginWithEmail에서 SDK 호출" "$(has_in_function $FBS 'loginWithEmail' 'signInWithEmailAndPassword')" "로그인이 SDK를 거치지 않으면 세션 생성 안 됨"
check "logout에서 SDK signOut 호출" "$(has_in_function $FBS 'Future<void> logout' 'signOut')" "signOut 없이는 SDK 세션이 디스크에 남음"
...
check "firebase_auth 의존성" "$(has_pattern $PUB 'firebase_auth')" "v3.5 인증 SDK"
